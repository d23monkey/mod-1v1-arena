...
public:
    ...
    void OnLogin(Player* pPlayer) override
    {
        if (sConfigMgr->GetOption<bool>("Arena1v1.Announcer", true))
            ChatHandler(pPlayer->GetSession()).SendSysMessage("本服务器正在运行 |cff4CFF001v1 |r模块.");
    }
...
bool npc_1v1arena::OnGossipHello(Player* player, Creature* creature)
{
    ...
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "退出1v1竞技场队列", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_LEAVE_QUEUE, "你确定吗?", 0, false);
    }
    else
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "加入1v1竞技场队列（非评级）", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_JOIN_QUEUE_ARENA_UNRATED);
    }

    if (!teamExistForPlayerGuid(player))
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "创建新的 1v1 竞技场队伍", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_CREATE_ARENA_TEAM, "你确定吗?", sConfigMgr->GetOption<uint32>("Arena1v1.Costs", 400000), false);
    }
    else
    {
        if (!player->InBattlegroundQueueForBattlegroundQueueType(bgQueueTypeId))
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "加入 1v1 竞技场(评级)", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_JOIN_QUEUE_ARENA_RATED);
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "清除竞技场队伍", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_DISBAND_ARENA_TEAM, "你确定吗?", 0, false);
        }

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "显示您的统计信息", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_GET_STATISTICS);
    }
...
bool npc_1v1arena::OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
{
    ...
    switch (action)
    {
        case NPC_ARENA_1V1_ACTION_CREATE_ARENA_TEAM:
        {
            ...
            {
                handler.PSendSysMessage("您必须达到 %u + 级才能创建 1v1 竞技场队伍.", sConfigMgr->GetOption<uint32>("Arena1v1.MinLevel", 70));
                return true;
            }
        ...
        case NPC_ARENA_1V1_ACTION_JOIN_QUEUE_ARENA_RATED:
        {
            if (Arena1v1CheckTalents(player) && !JoinQueueArena(player, creature, true))
                handler.SendSysMessage("加入队列时出错.");
